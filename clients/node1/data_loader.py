"""
Data loader for node1.
Updated to load from: data/federated/splits/iid_5nodes/node_1/
"""
import os
import torch
...
def get_loaders(images_dir=None, labels_dir=None, batch_size=16, val_frac=0.1, seed=42):
    ...
    if images_dir is None:
        # NEW PATH: data/federated/splits/iid_5nodes/node_1/
    ...
    print(f"[Node1 DataLoader] Loading data from:")
    ...
    print(f"[Node1 DataLoader] Total samples: {total}")
    ...
    if train_count <= 0:
        print("[Node1 DataLoader] Warning: Dataset too small, using all for train and val")
        train_ds = ds
        val_ds = ds
    else:
        train_ds, val_ds = random_split(ds, [train_count, val_count], generator=generator)
    
    print(f"[Node1 DataLoader] Train: {len(train_ds)}, Val: {len(val_ds)}")
    ...
    print("[Node1 DataLoader] ✓ Loaders created successfully")
...
if __name__ == "__main__":
    print("Testing node1 data_loader...")
    try:
